...
import { PreventivaProps } from "./tabelaPreventiva";

export function MobilePreventivaTableCards({
  item,
}: {
  item: PreventivaProps;
}) {
      ...
      <CardContent className="p-4">
        ...
        <div className="flex justify-between items-center mb-3">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent
              align="end"
              className={` ${color.textBranco} border-gray-700`}
            >
        ...
        {/* Equipamento e Local */}
        <div className="mb-4">
          <h3 className={`font-bold text-sm ${color.textBranco}`}>
            {item.equipamento}
          </h3>
        ...
        <div className="flex justify-between items-end mb-4">
          ...
          <div className="text-right">
            ...
            {!isFinalizado && (
              <div
                className={`flex items-center justify-end gap-1 text-[10px] font-bold ${agendamento.color}`}
              >
        ...
        <div className="pt-3 border-t border-gray-400/10">
          <div className="flex justify-between items-center mb-2">
            ...
            <span
              className={`text-xs font-bold ${isFinalizado ? "text-green-500" : "text-blue-400"}`}
            >
              {item.progresso}%
            </span>
          </div>
          ...
          <div className="mt-3 flex items-center justify-center">
            {isFinalizado ? (
              <div className="flex items-center gap-1.5 text-green-500 bg-green-500/10 px-3 py-1 rounded-full text-[11px] font-bold border border-green-500/20">
                <CheckCircle2 size={14} /> Finalizado em{" "}
                {formatarData(item.dataFinalizado as Date)}
              </div>
            ) : item.progresso > 0 ? (
