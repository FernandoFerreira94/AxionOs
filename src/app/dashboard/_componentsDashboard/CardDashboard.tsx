...
export function CardDashboard() {
  return (
    <section className="w-full grid grid-cols-5 gap-6 max-sm:gap-2 mt-4 max-sm:mt-0 max-sm:grid-cols-1 ">
        ...
        return (
          <Card key={status} className={`${config.text} max-sm:py-4`}>
            <CardContent className="flex flex-col gap-2 max-sm:flex-row max-sm:justify-between">
              ...
              <p className={`text-2xl font-semibold max-sm:text-xl ${color.textBranco}`}>0</p>
            </CardContent>
          </Card>
        );
      })}
